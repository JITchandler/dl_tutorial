...
## 我们将通道维移动到最后一维，因为在不同尺度下批量大小仍然保持不变
...
## 完整的模型
## 完整的SSD模型由五个模块组成，每个模块既能用于生成锚框，也能预测锚框的类别和偏移量
## 在五个模块中，第一个是基本网络块，第二个和第四个是高，宽减半块
## 最后一个模块使用全局最大池将高度和宽度都降到1
## 从技术上面来讲，第二到第五个区块都是多尺度特征块
...
## 为每一个块定义前向传播
## 输出包括：CNN特征图Y，当前尺度下根据Y生成的锚框，预测这些锚框的类别的偏移量
